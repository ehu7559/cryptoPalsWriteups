...
from s4c28 import SHA1_keyed_MAC, SHA1Digest, encode_uint_big_endian
from random import randint

#Validation Function
def validate_SHA1_keyed_MAC(key, message, hash_str):
    _, real_hash = SHA1_keyed_MAC(key, message)
    return real_hash == hash_str

#Oracle generator
def get_oracle(key):
    return lambda m, h : validate_SHA1_keyed_MAC(key, m, h)
...
def attack(oracle, message : bytes, hash_string : str, message_tail : bytes, max_depth = 65535):
    ...
    for key_length in range(max_depth):
        ...
        digest = SHA1Digest.from_hash_str(hash_string)
...
if __name__ == "__main__":
    ...
    chall_message, chall_init_hash = SHA1_keyed_MAC(oracle_key, chall_message)
    ...
    check_msg, check_hash = SHA1_keyed_MAC(oracle_key, forged_message)
